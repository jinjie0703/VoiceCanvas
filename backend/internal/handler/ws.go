...
func (h *WebSocketHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if h.enhancer != nil {
			...
			if err == nil {
				if !enhRes.IsValid {
					...
					h.sendFeedback(conn, &writeMu, enhRes.FeedbackMsg)
					continue
				}
		...
		if h.agent != nil {
			...
			h.agent.Run(ctx, finalPrompt, clientMsg.CanvasState,
...
func (h *WebSocketHandler) sendFeedback(conn *websocket.Conn, mu *sync.Mutex, message string) {
