...
func (a *Agent) Run(ctx context.Context, enhancedPrompt string, canvasState []model.CanvasElement, onAction OnActionCallback, observe ObservationFunc) {
	...
	for round := 0; round < a.maxRounds; round++ {
		...
		slog.Info("Agent response", "round", round+1, "content_len", len(content))

		// Parse response lines as JSONL
		actions, isDone := a.parseResponse(content, onAction)

		if isDone || len(actions) == 0 {
...
// parseResponse parses the LLM's JSONL output, sends actions via callback, and detects completion.
func (a *Agent) parseResponse(content string, onAction OnActionCallback) ([]model.DrawAction, bool) {
