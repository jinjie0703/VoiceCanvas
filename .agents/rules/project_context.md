...
3. **TypeScript 类型规范 (绝对禁止使用 any)**：【重点强调】严格遵守业界规范，**禁止在 TypeScript 代码中使用 `any` 类型**。若类型不确定，必须使用 `unknown` 并进行类型收窄，或通过泛型工具（如 `Record<string, unknown>`）提取精确类型。必须保证项目无 `@typescript-eslint/no-explicit-any` 报错。
